...
(function($) {
    var $window = $(window),
        ...
        defaults = {
            ...
            spacingWidth: 10, // the brick element horizontal spacing水平
            spacingHeight: 10, // the brick element vertical spacing垂直
            minColCount: 2, // min columns 最小行
    ...
    Waterfall.prototype = {
        ...
        _getColumnCount: function () {
            ...
            console.log(iCol);//4
            console.log(itemWidth);//262

            iCol = iCol > this.options.minColCount ? iCol : this.options.minColCount;
            realWidth = iCol * itemWidth;//1048

            if(parentWidth > realWidth) {
                leftOffset = Math.floor((parentWidth - realWidth - iCol * this.options.spacingWidth) / 2);//51
            }
        ...
        _positionAll: function () {
            ...
            $item.each(function(index) {
                ...
                if(index < $this.cols) {
                    ...
                    console.log('zuo:'+$this.leftOffset + index * $this.itemWidth + index * $this.options.spacingWidth);
                    $this.colHeightArray.push($(this).outerHeight());//高度传入到数组中
                } else {
                    minHeight = Math.min.apply(null, $this.colHeightArray);
                    minIndex = $.inArray(minHeight, $this.colHeightArray);
                    console.log(minIndex);//0,1,2,3
                    $(this).css("top", minHeight + $this.options.spacingHeight);
                    // $(this).css("left", $item.eq(minIndex).offset().left);

                    $(this).css("left", $this.leftOffset + count * $this.itemWidth + count * $this.options.spacingWidth);
                    count = ++count %5;
                    console.log($item.eq(minIndex).offset().left);
